...
export const data = new SlashCommandBuilder()
  .setName('front')
  .setDescription('Add a front bait to the shared bait list (owner only)')
  .addStringOption(option =>
    option.setName('name').setDescription('Name of the front bait').setRequired(true)
  )
...
export async function execute(interaction: CommandInteraction): Promise<void> {
  ...
  try {
    ...
    const existing = await getBaitByName(name, 'front');
    if (existing) {
      throw new Error(`A front bait named "${name}" already exists`);
    }

    await validateAudioAttachment(attachment);

    const jobId = `front-${Date.now()}`;
    ...
    const storageDir = path.join(config.storage.path, 'front-baits');
    ...
    const embed = createSuccessEmbed(
      '🎬 Front Bait Added',
      `Successfully added "${name}" to the front bait list.`
    ).addFields(
    ...
    logger.info({ baitId: bait.id, name }, 'Front bait added successfully');
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Unknown error';
    const embed = createErrorEmbed('Error', message);
    await interaction.editReply({ embeds: [embed] });
    logger.error({ error, name }, 'Failed to add front bait');
  }
}
